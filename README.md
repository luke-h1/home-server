...
| Prometheus, Grafana, Alertmanager    | `monitoring` | Separate ingress hostnames; scrapes **Immich** metrics (`immich-server.immich:8081`) with labels expected by community dashboards; **node-exporter**, **fail2ban-exporter** DaemonSet (host `/var/run/fail2ban`; requires OS **fail2ban**, default in `scripts/setup-server`), **kube-state-metrics**, **blackbox-exporter**; cAdvisor via API server for [K8S dashboard 15661](https://grafana.com/grafana/dashboards/15661-k8s-dashboard-en-20250125/); Grafana **Applications**: [Immich Overview 22555](https://grafana.com/grafana/dashboards/22555-immich-overview/); **Infrastructure**: Node Exporter, fail2ban, Blackbox, 15661, **Server security signals** (`f2b_*` + host metrics; not raw auth logs) |
